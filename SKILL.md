...
description: >
  ...
  覆盖3种教学模式：交互（决策模拟/多角色辩论/反事实推演）、
  生成（人物传记/事件分析/主题研究/考异溯源/社媒文案）、逐章解读（整卷逐事件条目深度解读）。
  当用户要求讲解/教学/学习/分析资治通鉴相关历史内容时触发。
  也触发于讨论历史人物生平、政治斗争案例分析、战略决策模拟等场景。
  Do NOT use when: 用户要苏格拉底式逐段导读/陪读/精读原文（应使用 zztj-guide）；
  用户只是简单查询一个事实（应使用 zztj-query），
...
| `python zztj.py chapter-events <n>` | 卷内事件条目清单（逐章解读选条用） | `chapter-events 11` |
...
| 逐事件条目解读整卷 | `逐章解读` | 逐章/整卷/读第X卷/卷11/把某卷讲完 |
...
| **逐章解读** | 原生成子模式 F | 以卷为单位逐事件条目解读，跳过非政治条目，进度追踪 |
...
Step 6: Agent 逐段导读（三维追问框架）：
        ...
        - 每层抛 2-3 个分析切入点（事实层/人性层/政治斗争层/现实映射层），用户可任选回答
        - 层内多角度处理（见 prompts/guide.md「多切入点处理机制」）：
          回应已答角度（不评判，深入1层）→ 补解读未答角度（精炼点睛≤50字/角度，标注【AI补充解读】）
          → 再进入下一层；用户全部未答则走卡点熔断
        - 揭示：人性规律 + 政治逻辑 + 现实启示（标注【现代类比】）
        - 史学扩展阅读（AI 补充，不提问）：司马光剪裁/对照正史/资治目的，标注【Agent知识】
        - 进入下一段
        卡点熔断：某层答不上来→给提示→给译文→直接揭示，不硬推三层。
        长度约束：只限 AI 输出（追问≤150字、补解读≤50字/角度、揭示≤400字、史学扩展≤200字），原文展示不限。
...
### 导读关键约束
- 逐层推进不跨层；层内可抛多角度，用户未答角度由 AI 精炼补解读，不丢弃
- 追问基于原文，不是空泛的"你觉得呢"
- 未答角度补解读标注【AI补充解读】，与用户洞察显式区分
- **主线聚焦人性/政治斗争/现实映射，不陷入史学考据**
- **现实映射偏实用（职场/管理/人际/决策），不做空泛思辨**
- **史学层由 AI 补充为扩展阅读，不向用户提问**
- 难度根据用户对古文的熟悉程度自动调整
- 定档后严格按档位走，不中途跳档
- **【强制归档】每完成一段导读（揭示+史学扩展输出完毕），必须先执行 Step 7-9 完成归档（写 guide 文件 + 更新 index.json + 更新 progress.json），确认归档成功后再进入下一段。禁止跳过归档直接开始下一段。**
...
## 模式 4：逐章解读（整卷逐事件条目深度解读）

**核心**：以卷为单位，对卷内**每个事件条目**逐一深度解读，跳过星象/气候/灾异等非政治条目，按事件记录生成轨迹。**独立模块**，详细流程见 `prompts/chapter.md`。

### 粒度约定（关键）
数据库 `sentences` 表中，每段（年）内以 `[N]` 前缀标记事件条目，N 从 1 重新编号。一个 `[N]` 及其后的无标记句子 = 一个独立事件条目。**不需要 AI 自行切分事件**，只需对每条做类型判定（解读 or 跳过）。

### 数据检索步骤
```
Step 1: python zztj.py chapter-events {卷}       → 全卷事件条目清单（年/条号/句子数/首句）
Step 2: Read teaching/reading/{卷}/events.json    → 查轨迹（已 done / 待解读）
Step 3: python zztj.py chapter {卷}               → 取选定条目的完整原文+译文
Step 4: [可选] python zztj.py event/graph/person  → 背景补充
```

### 教学执行
```
Step 5: Read prompts/chapter.md
Step 6: 两步法执行：
        第一步（首次读该卷）：运行 chapter-events 生成条目清单 → AI 对每条做类型判定
                （政治/军事/制度/外交/人事→待解读；星象/气候/灾异/纯礼仪→skip）
                → Write teaching/reading/{卷}/events.json（schema 见 prompts/chapter.md）
                → 展示用户每条名称+条号+类型+跳过建议，等用户确认
        第二步（逐条解读）：用户选 1-3 条 → 取原文+译文 → 三维框架深度解读
                → Write teaching/generate/{YYYYMMDD}-卷{卷}-年{年}条{条}-解读.md
                → 更新 events.json 该条 status=done
Step 7: 更新 teaching/index.json
```

### 三维洞察框架（每条解读骨架）
1. **事实层**【史实】：事件经过、时间地点、参与者、结果
2. **人性层**：关键人物内心真正想要什么？暴露什么性格/欲望/弱点
3. **政治斗争层**：各方利益、信息差、权力结构、司马光警示
4. **现实映射**【现代类比】：可迁移的底层逻辑
5. **司马光史笔**【Agent知识】：剪裁/对照正史/资治目的（AI补充）
6. **三层分离标注**：史实/史论/演义

### 跳过规则（硬性）
- 星象、气候、灾异（彗星/地震/大旱等）→ `skip: true`
- 纯礼仪/制度/孤句（如"大赦天下"、"薨"讣告）→ 并入相邻条目或跳过
- 跳过条目不产生解读文件，仅在 events.json 标记 `skip` + `reason`

### 关键约束
- 单条目解读 400~800 字（原文另附），一次默认 1~3 条
- 每条解读至少含人性层 + 政治斗争层 + 现实映射，不复述原文
...
- 逐章解读 → 解读文稿 `teaching/generate/{YYYYMMDD}-卷{卷}-年{年}条{条}-解读.md`（submode=逐章解读）；条目清单+轨迹 `teaching/reading/{卷}/events.json`（不入库，每次 Read → 更新 status → Write）
