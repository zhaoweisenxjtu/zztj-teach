...
description: >
  ...
  生成（人物传记/事件分析/主题研究/考异溯源/社媒文案/逐章解读）。
...
| `python zztj.py chapter-events <n>` | 卷内事件条目清单（逐事件解读选条用） | `chapter-events 11` |
...
## 模式 3：生成（人物传记 / 事件分析 / 主题研究 / 考异溯源 / 社媒文案 / 逐章解读）

**核心**：一次性产出完整文稿。六种生成子模式，根据用户意图自动选择。
...
**核心**：以卷为单位，对卷内**每个事件条目**逐一深度解读，跳过星象/气候/灾异等非政治条目，按事件记录生成轨迹。详见 `prompts/generate.md` 子模式 F。
...
- 生成 → `teaching/generate/`（文件名后缀区分子模式：传记/事件分析/主题研究/考异溯源/文案/**解读**）

逐章解读特殊路径：
- 解读文稿 → `teaching/generate/{YYYYMMDD}-卷{卷}-年{年}条{条}-解读.md`，submode=逐章解读
- 条目清单 + 轨迹 → `teaching/reading/{卷}/events.json`（不入库，每次 Read → 更新 status → Write）
